             ...
             <div class="modal" id="studentModal" role="dialog" style="z-index: 99999">
                        <div class="modal-dialog modal-lg">
							...
							<div class="col-lg-12 text-center " style="padding:10px 10px;">
										...
										<div class="col-md-8">
										  <textarea name="message" id="msgId" rows="4" cols="80" placeholder="Type the message for the student"></textarea>
										  
	<input type="hidden" id="orgId" value="">
	<input type="hidden" id="studentId" value="">
	<input type="hidden" id="enrollmentId" value=""></div>
	<div class="col-md-4">
    <button name="msgbtn" id="msgbtn" class="btn btn-info btn-lg"> Send Message </button>
	</div>
      ...
      <div class="modal-body">
	          <div class="col-lg-12 " style="padding:10px 10px;">
						...
						<button name="sndbtn" type="submit" id="sndbtn" class="msg_send_btn btn-info "> <i class="fa fa-paper-plane-o" aria-hidden="true"></i></button>
							...
							<input type="hidden" id="oId" name="oId" value="<?php echo $_SESSION['orgId']; ?>">
							<input type="hidden" id="msgFrom" name="msgFrom" value="<?php echo  $_SESSION['orgName']; ?>">
							</div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#details_table').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?php echo site_url('university/getEnrollApplications') ?>",
                "dataType": "json",
                "type": "POST"
            },
            "columns": [
	...
	function studentInfo(roleName, courseId, orgCourseId,studentId,enrollmentId)
    {
		...
		$('#enrollmentId').val(enrollmentId);
            ...
            $.ajax({
                type: "POST",
                data: {type: roleName, courseId: courseId, orgCourseId: orgCourseId,studentId:studentId},
                url: "<?php echo site_url('University/getEnrollData'); ?>",
                success: function (response) {
                    var json = $.parseJSON(response);
    ...
    $("#closestudentModal").click(function () {
	  ...
	  function showMsg(eId,sId,sName){
		  $('#sId').val(sId);
		  $('#eId').val(eId);
		  $('#msgTo').val(sName);
            ...
            $.ajax({
                type: "POST",
                data: {eId: eId},
                url: "<?php echo site_url('University/showMsg'); ?>",
                success: function (response) {
					...
					for (var i = 0; i < json.length ; i++) {
						...
						var status='<b style="color:#0a175a;"> '+json[i].status+'</b>';}
						 ...
						 $(".msg_history").append('<div class="outgoing_msg"><div class="sent_msg"> <p>'+json[i].msg+'&nbsp;&nbsp;<span class="time_date"> '+json[i].createdDate+'&nbsp;&nbsp;('+status+')</p></span></div></div>');
						}
						...
						{
						$(".msg_history").append('<div class="incoming_msg"> Message From <b style="color:darkBlue;">'+json[i].msgFrom +'</b> <div class="received_msg"> <div class="received_withd_msg"> <p>'+json[i].msg+'&nbsp;&nbsp;<br><a href="<?php echo base_url(); ?>'+json[i].docAttachment+'">'+attach+'</a><span class="time_date"> '+json[i].createdDate+'</span></p></div> </div></div>');
					}
					}
					}
}
...
$('#sndbtn').click(function(){
		 ...
		 if (orgId === "" && studentId === "" && enrollmentId === "" && msg === "" && msgFrom === "" && msgTo === "") {
		    ...
		    $.ajax({
                    url: "<?php echo site_url('university/notifyMsg'); ?>",
                    type: "POST",
