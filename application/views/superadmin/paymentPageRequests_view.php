            ...
            <div class="col-md-12 table-responsive">
                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title">Payment Asked Page Request Details</h3>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $.ajax({
            type: "POST",
            data: {pageStatusType: "askPayment"},
            ...
            success: function (response) {
                ...
                for (var i = 0; i < json.length; i++) {
                    ...
                    if (json[i].approvalStatus === 'askPayment') {
                        approvalReq = '<span class="btn btn-sm btn-success">Amount : ' + json[i].paymentAmount + '' + (json[i].paymentStatus === "1" ? " Paid" : " Not paid") + '</span>';
                    } else {
                        if (json[i].paymentAmount) {
                            approvalReq = approvalReq + '<span class="btn btn-sm btn-success">Amount : ' + json[i].paymentAmount + '</span>';
                        }
                    }
    ...
    function viewPage(pageId) {
        if (pageId !== "") {
            $.ajax({
                type: "POST",
                data: {pageId: pageId, pageStatusType: "askPayment"},
                url: "<?php echo site_url('superadmin/getPageRequests'); ?>",
                success: function (response) {
                    if (response !== "") {
                        var json = $.parseJSON(response);
...
<script type="text/javascript">
    ...
    $(".ppageReqLink").addClass("active");
    document.title = "iHuntBest | Page Payment Asked Details";
</script>