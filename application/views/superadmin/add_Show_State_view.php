...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">Add/Show States</li>
        </ol>
    </section>
                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title"><?php
                            ...
                            ?></h3>
                        <br><a href="<?php echo site_url("superadmin/addShowCountry"); ?>"> Go back to country</a>
                    </div>

<?php echo form_open_multipart("superadmin/insertState", ["id" => "submit_form"]); ?>
                    <div class="box-body">

                        <input type="hidden" name="id" id="id" value="no_one" >
                        <input type="hidden" name="countryId" id="country_Id" value="<?php
if (isset($countryid)) {
    echo $countryid;
}
?>" >
                        <input type="hidden" name="flagname" id="flagname" value="" >
                        <input type="hidden" name="countryname" value="<?php
                        if (isset($cname)) {
                            echo $cname;
                        }
?>">
...
<script type="text/javascript">
                                                   $(document).ready(function () {
                                                       $(document).on("click", ".editfunction", function () {
                                                           var stateId = $(this).attr('ed');
                                                           $.ajax({
                                                               type: "POST",
                                                               data: {stateid: stateId, countryId: '<?php
if (isset($countryid)) {
    echo $countryid;
}
?>'},
                                                               ...
                                                               success: function (response) {
                                                                   ...
                                                                   $('#id').val(json[0].stateId);
                                                                   $('#name').val(json[0].name);
                                                                   $('#flagname').val(json[0].state_flag);
                                                               }
                                                           });
                                                       });
                                                       $(document).on("click", ".delfunction", function () {
                                                           var stateid = $(this).attr('del');
                                                           $.confirm({
                                                               title: 'Warning!',
                                                               ...
                                                               buttons: {
                                                                   ...
                                                                   Confirm: function () {
                                                                       $.ajax({
                                                                           type: "POST",
                                                                           data: {stateid: stateid, ihuntcsrfToken: '<?php echo get_cookie('ihuntcsrfCookie'); ?>'},
                                                                           url: "<?php echo site_url('superadmin/delState'); ?>",
                                                                           success: function (response) {
                                                                               var json = $.parseJSON(response);
                                                       ...
                                                       $(document).on("change", ".state_status", function () {
                                                           var id = $(this).attr("id");
                                                           var state_status = $(this).val();
                                                           $.ajax({
                                                               type: "POST",
                                                               data: {id: id, state_status: state_status, ihuntcsrfToken: '<?php echo get_cookie('ihuntcsrfCookie'); ?>'},
                                                               url: '<?php echo site_url('superadmin/updateStatusState'); ?>',
                                                               success: function (response) {
                                                                   var json = $.parseJSON(response);
                                                                   if (json.status === 'success') {
                                                                       $.alert({title: 'Success!', content: json.msg, type: 'blue',
...
?>'},
                                                           ...
                                                           success: function (response) {
                                                               ...
                                                               for (var i = 0; i < json.length; i++) {
                                                                   if (json[i].stateFlag === "0") {
                                                                       var status = '<span class="label label-primary">Active</span>';
                                                                   } else if (json[i].stateFlag === "1") {
                                                                       var status = '<span class="label label-danger">Disable</span>';
                                                                   }
                                                                   flagimg = '<img src="<?php echo base_url('projectimages/images/stateflags/'); ?>' + json[i].state_flag + '" class="img-responsive" \n\
                    onerror="this.src=\'<?php echo base_url('plugins/no-image.png'); ?>\'">';
                                                                   oTable.fnAddData([
                                                                       (i + 1),
                                                                       json[i].name,
                                                                       flagimg,
                                                                       '<a href="addCity?ed=' + Base64.encode(json[i].stateId) + '" title="More"><i class="fa fa-plus"></i></i></a>',
                                                                       '<select class="state_status" id="' + json[i].stateId + '"style="background-color:#acd6e9;"><option value="">Choose One...</option><option value="0">Active</option><option value="1">Disable</option></select>',
                                                                       status,

                                                                       '<a href="#" ed="' + json[i].stateId + '" class="editfunction" title="Edit"><i class="fa fa-edit"></i></i></a>&nbsp;&nbsp;&nbsp;&nbsp;<a class="delfunction" href="javascript:" del="' + json[i].stateId + '" title="Delete"><i class="fa fa-trash-o"></i></i></a>'
                                                                   ]);
                                                               }
                                                           }
                                                       });
...
<script type="text/javascript">
    ...
    document.title = "iHuntBest | State";
</script>