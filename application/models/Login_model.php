...
class Login_model extends CI_Model {
    ...
    public function mRegisterAdmin($roleType, $orgName, $orgMobile, $email, $password, $countryId, $stateId, $cityId, $address) {
        ...
        $chk_qry = $this->db->query("SELECT * FROM login_details WHERE email='" .$email."'");
        ...
        if ($chk_qry->num_rows() > 0) {
            ...
            $pwd_encrypt = $this->encryption->encrypt($password);

            $data = ["email" => $email, "password" => $password, "password1" => $pwd_encrypt,
				"roleName" => $roleType, "ipAddress" => $this->getRealIpAddr(), "createdAt" => $this->datetimenow(),
				"isactive" => 1];
