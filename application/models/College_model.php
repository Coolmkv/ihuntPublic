...
class College_model extends CI_Model {
    ...
    //College Dashboard Start
    public function mgetDashoboardDetails() {
        ...
        $res = $this->db->query("SELECT (SELECT COUNT(*)  FROM faculty_details WHERE loginId=" . $_SESSION['loginId'] . " and isactive=1) AS totalemployees,
                                    ...
                                    (SELECT COUNT(*) FROM tbl_enroll te INNER JOIN organization_streams os ON os.orgStreamId=te.orgStreamId
                                    AND os.isactive=1 WHERE te.isactive=1 AND os.loginId=" . $_SESSION['loginId'] . " ) as totalenroll   ");
        if ($res->num_rows() > 0) {
            return $res->row();
        } else {
            return false;
    ...
    public function getProfileInfo() {
        ...
        $qry = $this->db->query("SELECT ld.email,ld.id,od.orgName,od.orgType,od.orgMobile,od.orgAddress,od.orgLogo,od.orgImgHeader,
                                ...
                                od.cityId,od.orgEstablished ,od.org_landline FROM login_details as ld
    ...
    public function mupdateProfile($directorName, $directorMobile, $directorEmail, $orgName, $orgMobile, $countryId, $stateId, $cityId, $orgType, $orgGoogle, $orgWebsite, $approvedBy, $orgaddress, $edate) {
        ...
        if (empty($directorName) || empty($directorMobile) || empty($directorEmail) || empty($orgName) || empty($orgMobile) || empty($countryId) || empty($stateId) || empty($cityId) || empty($orgType) || empty($orgWebsite) || empty($approvedBy) || empty($orgaddress) || empty($orgDesp) || empty($orgMission) || empty($orgVission) || !isset($_SESSION['loginId'])) {
            ...
            $data = ["orgName" => $orgName, "directorName" => $directorName, "directorMobile" => $directorMobile, "directorEmail" => $directorEmail, "orgMobile" => $orgMobile, "org_landline" => $org_landline, "countryId" => $countryId, "stateId" => $stateId, "cityId" => $cityId, "orgType" => $orgType, "orgGoogle" => $orgGoogle, "orgWebsite" => $orgWebsite, "orgEstablished" => $edate,
                "approvedBy" => $approvedBy, "orgAddress" => $orgaddress, "orgDesp" => $orgDesp, "orgMission" => $orgMission, "orgVission" => $orgVission, "orgUpdated" => $this->datetimenow()];
    ...
    public function mSaveFinancialDetails($respond) {
        ...
        if (empty($bank_name) || empty($accountType) || empty($account_holder_name) || empty($baddress) || empty($accno) || empty($micrc) || empty($ifscc)) {
            return '{"status":"error","msg":"Required field is empty"}';
        } else {
...
//            $dcardcvv = ($dcardcvv === "" ? NULL : $this->encryption->encrypt($dcardcvv));
            ...
            $idata = ["credit_card_no" => $ccardno, "debit_card_no" => $dcardno, "bank_name" => $bank_name, "bankaddress" => $baddress, "bank_account_no" => $accno,
                "account_holder_name" => $account_holder_name, "ifscCode" => $ifscc, "micr_code" => $micrc, "login_id" => $_SESSION['loginId'], "defaultCurrency" => $defaultCur, "user_type" => "College",
                "ip_address" => $this->getRealIpAddr(), "created_on" => $this->datetimenow(), "accountType" => $accountType, "isactive" => 1];
            $resp = $this->db->insert("tbl_financial_details", $idata);
        }
        ($resp ? $this->addActivityLog($_SESSION['loginId'], "Finance Detail Inserted for College by " . $_SESSION['orgName'], "tbl_financial_details", "0") : '');
    ...
    public function upDateFinancialDetails($id, $ccardno, $dcardno, $bank_name, $baddress, $accno, $ifscc, $micrc, $defaultCur, $respond, $account_holder_name, $accountType) {
        $details = $this->db->where(["financial_detail_id!=" => $id, "login_id" => $_SESSION['loginId'], "user_type" => "College"])->get("tbl_financial_details");
        if ($details->num_rows() > 0) {
            ...
            $idata = ["credit_card_no" => $ccardno, "debit_card_no" => $dcardno, "bank_name" => $bank_name, "bankaddress" => $baddress, "bank_account_no" => $accno, "account_holder_name" => $account_holder_name, "ifscCode" => $ifscc, "micr_code" => $micrc,
                "login_id" => $_SESSION['loginId'], "defaultCurrency" => $defaultCur, "user_type" => "College", "ip_address" => $this->getRealIpAddr(), "updated_on" => $this->datetimenow(), "accountType" => $accountType];

            $resp = $this->db->where("financial_detail_id", $id)->update("tbl_financial_details", $idata);
        }
        ($resp ? $this->addActivityLog($_SESSION['loginId'], "Finance Detail Updated for College by " . $_SESSION['orgName'], "tbl_financial_details", "0") : "");
    ...
    private function uploadVideo($path, $vidName, $uploadfname) {
        ...
        $config = ['upload_path' => $path, 'allowed_types' => "mp4|mkv|m4v|video\/x-matroska|avi|flv|mov|quicktime|MP4|3gp|3GP|flv|.mkv|FLV|video", "file_name" => $vidName];
        $this->load->library('upload', $config);
        if ($this->upload->do_upload($uploadfname) == false) {
            ...
            $headerVideoUrl = FILTER_VAR(trim($this->input->post('headerVideoUrl')), FILTER_SANITIZE_STRING);
            ($headerVideoUrl != "" ? unlink($headerVideoUrl) : "");
...
//            $error = array('error' => $this->upload->display_errors());
    ...
    public function mchangePasswordSave() {
        ...
        if ($newpass !== $cnewpas) {
            ...
            if ($currpass !== $oldpass && $oldpass !== $data->password) {
                ...
                $uData = ["password" => $cnewpas, "password1" => $this->encryption->encrypt($newpass), "updatedAt" => $this->datetimenow()];
    ...
    public function mGetTimeDurationArr() {
        if (!isset($_SESSION['loginId'])) {
            return '{"status":"error", "msg":"Unauthorised access!"}';
        }
        $qry = $this->db->query("SELECT * FROM time_duration where isactive=1 order by title ASC");
    ...
    public function mGetMinQualification() {
        ...
        return json_encode(($qry->num_rows() > 0 ? $qry->result() : ""));
    }
    ...
    public function mGetCourseTypeArr() {
        if (!isset($_SESSION['loginId'])) {
            return '{"status":"error", "msg":"Unauthorised access!"}';
        }
        $qry = $this->db->query("SELECT * FROM course_type where isactive=1 order by courseType ASC");
    ...
    public function maddCourse($orgCourseId) {
        ...
        if (!isset($_SESSION['loginId']) || empty($courseDurationType) || empty($courseTypeId) || empty($courseId) || empty($timeDurationId)) {
            return '{"status":"error","msg":"Required field is empty!"}';
        } else {
            return ($orgCourseId == 'no_one' ? $this->addNewCourse($_SESSION['loginId'], $courseTypeId, $courseId, $timeDurationId, $this->datetimenow(), $courseDurationType, $minage, $maxage, $validupto, $openingDate, $closingDate, $applicationFee, $examMode, $examDetails, $examDate, $resultDate) :
                    $this->upDateCourse($_SESSION['loginId'], $orgCourseId, $courseTypeId, $courseId, $timeDurationId, $this->datetimenow(), $courseDurationType, $minage, $maxage, $validupto, $openingDate, $closingDate, $applicationFee, $examMode, $examDetails, $examDate, $resultDate));
        }
    }

    private function addNewCourse($loginId, $courseTypeId, $courseId, $timeDurationId, $datetimenow, $courseDurationType, $minage, $maxage, $validupto, $openingDate, $closingDate, $applicationFee, $examMode, $examDetails, $examDate, $resultDate) {
        $chkData = ["loginId" => $loginId, "courseTypeId" => $courseTypeId, "courseId" => $courseId, "courseDurationId" => $timeDurationId, "courseDurationType" => $courseDurationType, "isactive" => 1];

        $duplicate = $this->db->where($chkData)->get("organization_courses");
        if ($duplicate->num_rows() > 0) {
            return '{"status":"error", "msg":"Duplicate entry!"}';
        } else {
            $iData = ["loginId" => $loginId, "courseTypeId" => $courseTypeId, "courseId" => $courseId, "courseDurationId" => $timeDurationId, "courseDurationType" => $courseDurationType, "openingDate" => $openingDate, "closingDate" => $closingDate,
                "applicationFee" => $applicationFee, "examDate" => $examDate, "examMode" => $examMode, "examDetails" => $examDetails, "resultDate" => $resultDate, "minAge" => $minage, "maxAge" => $maxage, "ageValidDate" => $validupto, "createdAt" => $datetimenow, "isactive" => 1];
            $res = $this->db->insert("organization_courses", $iData);
            $orgCourseId = $this->db->insert_id();
            $this->eligibilityInsertUpdate($orgCourseId);
            $this->experienceInsertUpdate($orgCourseId);
            ($res ? $this->addActivityLog($_SESSION['loginId'], "Organization Course Details Inserted by " . $_SESSION['orgName'], "organization_courses", "0") : "");
            return ($res ? '{"status":"success","msg":"Saved Successfully"}' : '{"status":"error","msg":"Error in server, please contact admin!"}');
        }
    }

    private function eligibilityInsertUpdate($orgCourseId) {
        ...
        $chk = $this->db->where("orgCourseId", $orgCourseId)->get("orgcourse_mapping_prerequisites");
        if ($chk->num_rows() > 0) {
            $this->db->where("orgCourseId", $orgCourseId)->update("orgcourse_mapping_prerequisites", ["isactive" => 0]);
        }
        for ($mq = 0; $mq < count($min_qualification); $mq++) {
            $this->minsertUpdatePrerequisites($mq, $min_qualification, $min_qualificationStream, $condition, $orgCourseId, $min_percentage, $markingType);
        }
        return true;
    }
    ...
    private function experienceInsertUpdate($norgCourseId) {
        ...
        for ($edt = 0; $edt < count($duration_type); $edt++) {
            ...
            if ($chkomp->num_rows() > 0) {
                $udata = ["orgCourseId" => $norgCourseId, "expDurationType" => $duration_type[$edt], "expDurationId" => $experience_duration[$edt], "description" => $description[$edt], "updatedAt" => $this->datetimenow(), "isactive" => 1];
                $this->db->where("experienceId", $chkomp->row()->experienceId)->update("orgcourse_mapping_experience", $udata);
            } else {
                $edtData = ["orgCourseId" => $norgCourseId, "expDurationType" => $duration_type[$edt], "expDurationId" => $experience_duration[$edt], "description" => $description[$edt], "createdAt" => $this->datetimenow(), "isactive" => 1];
                $this->db->insert("orgcourse_mapping_experience", $edtData);
            }
        }
    }

    private function upDateCourse($loginId, $orgCourseId, $courseTypeId, $courseId, $timeDurationId, $datetimenow, $courseDurationType, $minage, $maxage, $validupto, $openingDate, $closingDate, $applicationFee, $examMode, $examDetails, $examDate, $resultDate) {
        $chkData = ["orgCourseId!=" => $orgCourseId, "loginId" => $loginId, "courseTypeId" => $courseTypeId, "courseId" => $courseId, "courseDurationId" => $timeDurationId,
            "applicationFee" => $applicationFee, "courseDurationType" => $courseDurationType, "isactive" => 1];

        $duplicate = $this->db->where($chkData)->get("organization_courses");
        if ($duplicate->num_rows() > 0) {
            return '{"status":"error", "msg":"Duplicate entry!"}';
        } else {
            $uData = ["courseTypeId" => $courseTypeId, "courseId" => $courseId, "courseDurationId" => $timeDurationId, "courseDurationType" => $courseDurationType, "openingDate" => $openingDate, "closingDate" => $closingDate, "examDate" => $examDate,
                "applicationFee" => $applicationFee, "examDate" => $examDate, "examMode" => $examMode, "examDetails" => $examDetails, "resultDate" => $resultDate, "minAge" => $minage, "maxAge" => $maxage, "ageValidDate" => $validupto, "updatedAt" => $datetimenow];


            $res = $this->db->where(["orgCourseId" => $orgCourseId, "loginId" => $loginId, "isactive" => 1])->update("organization_courses", $uData);
            $this->eligibilityInsertUpdate($orgCourseId);
            $this->experienceInsertUpdate($orgCourseId);
            ($res ? $this->addActivityLog($_SESSION['loginId'], "Organization Course Details Updated by " . $_SESSION['orgName'], "organization_courses", "0") : "");
            return ($res ? '{"status":"success","msg":"Saved Successfully"}' : '{"status":"error","msg":"Error in server, please contact admin!"}');
        }
    }
    ...
    public function checkCourseType($title) {
    ...
    public function mGetCourses() {
        ...
        $condtion = ($orgCourseId != "" ? "AND oc.orgCourseId =" . $orgCourseId . "" : "");
        $qry = $this->getCourses($condtion);
        if ($qry->num_rows() > 0) {
            foreach ($qry->result() as $rs) {
                $rs->minqual = $this->getMinQualificationDetails($rs->orgCourseId);
            }
        }
    ...
    public function mEditCourses() {
        ...
        $qry = $this->db->query("SELECT oc.*,(SELECT GROUP_CONCAT(CONCAT(omp.prerequisitesId,',',omp.courseMinQualId,',',
            omp.tableName,',', omp.minqualstreamId,',',omp.streamTableName,',',omp.markingType,',', omp.percentage,',',
            omp.relationType) SEPARATOR '^')  FROM orgcourse_mapping_prerequisites omp WHERE omp.orgCourseId=$orgCourseId
            AND omp.isactive=1 GROUP by omp.orgCourseId) prereqdetails ,(SELECT GROUP_CONCAT(CONCAT(ome.expDurationType,',',
            ome.expDurationId,',', ome.description) SEPARATOR '^') FROM orgcourse_mapping_experience
            ome WHERE ome.orgCourseId=$orgCourseId AND ome.isactive=1 GROUP BY ome.orgCourseId) expDetails FROM organization_courses oc
            WHERE oc.orgCourseId=$orgCourseId AND oc.isactive=1");
    ...
    private function getMinQualificationDetails($orgCourseId) {
        $return = "";
        if (empty($orgCourseId)) {
            return $return;
        }
        $qry = $this->db->query("SELECT omp.prerequisitesId,cd.title courseName,sd.title streamName,omp.markingType,omp.percentage,omp.relationType FROM orgcourse_mapping_prerequisites omp
                        INNER JOIN course_details cd ON cd.cId=omp.courseMinQualId AND omp.tableName='course_details' INNER JOIN stream_details sd ON sd.streamId=omp.minqualstreamId AND omp.streamTableName='stream_details'
                        WHERE omp.orgCourseId=$orgCourseId AND omp.isactive=1 UNION SELECT omp.prerequisitesId,tc.classTitle courseName,sct.title streamName,omp.markingType,omp.percentage,omp.relationType FROM orgcourse_mapping_prerequisites omp
                        INNER JOIN tbl_classnames tc ON tc.classnamesId=omp.courseMinQualId AND omp.tableName='tbl_classnames'INNER JOIN school_class_type sct ON sct.classTypeId=omp.minqualstreamId AND omp.streamTableName='school_class_type'  WHERE omp.orgCourseId=$orgCourseId AND omp.isactive=1   UNION
                        SELECT omp.prerequisitesId,ic.title courseName,'Not Available' streamName,omp.markingType,omp.percentage,omp.relationType FROM orgcourse_mapping_prerequisites omp
                        INNER JOIN institute_course ic ON ic.insCourseId=omp.courseMinQualId AND omp.tableName='institute_course'  WHERE omp.orgCourseId=$orgCourseId AND omp.isactive=1       UNION
                        SELECT omp.prerequisitesId,tcem.exam_name courseName,'Not Available' streamName,omp.markingType,omp.percentage,omp.relationType FROM orgcourse_mapping_prerequisites omp
                        INNER JOIN tbl_competitive_exam_master tcem ON tcem.c_exam_id=omp.courseMinQualId AND omp.tableName='tbl_competitive_exam_master' WHERE omp.orgCourseId=$orgCourseId AND omp.isactive=1
                        UNION SELECT omp.prerequisitesId,tsm.subjectId courseName,'Not Available' streamName,omp.markingType,omp.percentage,omp.relationType FROM orgcourse_mapping_prerequisites omp
                        INNER JOIN tbl_subjectmaster tsm ON tsm.subjectId=omp.courseMinQualId AND omp.tableName='tbl_subjectmaster' WHERE omp.orgCourseId=$orgCourseId AND omp.isactive=1");
        if ($qry->num_rows() > 0) {
            foreach ($qry->result() as $rs) {
                $return = $return . ' Course Name:' . $rs->courseName . '<br>Stream Name:' . $rs->streamName . '<br>' . $rs->markingType . ':' . $rs->percentage . ' ' . $rs->relationType;
            }
            return $return;
        }
    }
    ...
    private function getCourses($condtion) {
        return $this->db->query("SELECT oc.orgCourseId,oc.courseId,oc.*,'' minqual,cft.FeeType_Name as courseFeeType,ct.courseType,cd.title as course,td.title as timeduration,
                                    oc.courseTypeId, oc.courseDurationId,cft.courseFeeType_Id,oc.courseDurationType,(SELECT GROUP_CONCAT(ome.expDurationType) FROM orgcourse_mapping_experience ome
                                WHERE ome.orgCourseId=oc.orgCourseId AND ome.isactive=1) expDurationTypes,oc.openingDate,oc.closingDate,CONCAT('Application Start On: ',DATE_FORMAT(oc.openingDate,'%d-%b-%Y'),
                                ' and Ends On: ',DATE_FORMAT(oc.closingDate,'%d-%b-%Y')) impDates,(SELECT GROUP_CONCAT(ome.expDurationId) FROM orgcourse_mapping_experience ome WHERE ome.orgCourseId=oc.orgCourseId AND ome.isactive=1) expDurationIds,
                                (SELECT GROUP_CONCAT(ome.description) FROM orgcourse_mapping_experience ome WHERE ome.orgCourseId=oc.orgCourseId AND ome.isactive=1) descriptions,
                                (SELECT GROUP_CONCAT(td1.title) FROM orgcourse_mapping_experience ome LEFT JOIN time_duration td1 ON td1.tdId=ome.expDurationId AND td1.isactive=1
                                 WHERE ome.orgCourseId=oc.orgCourseId AND ome.isactive=1) timeDurationtitles FROM organization_courses as oc  INNER JOIN course_type as ct ON ct.ctId=oc.courseTypeId
                                 INNER JOIN course_details as cd ON cd.cId=oc.courseId  INNER JOIN time_duration as td ON td.tdId=oc.courseDurationId  LEFT JOIN course_fee_type cft ON cft.courseFeeType_Id=oc.courseFeeType
                                WHERE oc.isactive=1 and oc.loginId =" . $_SESSION['loginId'] . "  $condtion ");
    }
    ...
    public function mdeleteCourse() {
        ...
        if ($chk->num_rows() > 0) {
            ...
            if ($response) {
                $this->addActivityLog($_SESSION['loginId'], "Organization Course Details Deleted by " . $_SESSION['orgName'], "organization_courses", "0");
                return '{"status":"success", "msg":"Deletion Successful"}';
            } else {
                return '{"status":"error", "msg":"Error in server, please contact admin!"}';
            }
    ...
    public function mgetCourseDetails($orgCourseId) {
        $orgId = $_SESSION['loginId'];
        return $this->db->query("SELECT oc.*, (SELECT GROUP_CONCAT(ompre.courseMinQualId) FROM organization_courses orgc
                LEFT JOIN orgcourse_mapping_prerequisites ompre ON ompre.orgCourseId=orgc.orgCourseId AND ompre.isactive=1
                         ...
                         ) as courseMinQualIds,(SELECT GROUP_CONCAT(ompre.percentage) FROM organization_courses orgc
                         LEFT JOIN orgcourse_mapping_prerequisites ompre ON ompre.orgCourseId=orgc.orgCourseId AND ompre.isactive=1
                          WHERE orgc.orgCourseId=$orgCourseId AND orgc.isactive=1 AND orgc.loginId=$orgId GROUP BY orgc.orgCourseId
                         ) as percentages,GROUP_CONCAT(ome.expDurationType) as expDurationTypes,GROUP_CONCAT(ome.expDurationId) AS expDurationIds,
...
//            return $this->db->query("SELECT oc.*, GROUP_CONCAT(DISTINCT omp.courseMinQualId) as courseMinQualIds,GROUP_CONCAT(DISTINCT ome.orgCourseId) as experienceId,
//                            GROUP_CONCAT(DISTINCT omp.percentage) as percentages FROM organization_courses oc
//                            LEFT JOIN orgcourse_mapping_prerequisites omp ON omp.orgCourseId=oc.orgCourseId AND omp.isactive=1
//                            LEFT JOIN orgcourse_mapping_experience ome ON ome.orgCourseId=oc.orgCourseId AND ome.isactive=1
//
//                            WHERE oc.isactive=1 AND oc.orgCourseId=$orgCourseId AND oc.loginId=$orgId GROUP BY oc.orgCourseId");
    }
    ...
    public function maddNewStreams($orgStreamId, $orgCourseIds, $streamId, $description) {
        ...
        return $this->insertUpdateStreams($this->datetimenow(), $orgStreamId, $orgCourseId, $streamId, $_SESSION['loginId'], $description, $totalSheet, $seoKeywords, $availableSheet, $courseFeeType, $courseFee, $registrationFee, $fromDate, $toDate);
    }

    private function insertUpdateStreams($timenow, $orgStreamId, $orgCourseId, $streamId, $loginId, $description, $totalSheet, $seoKeywords, $availableSheet, $courseFeeType, $courseFee, $registrationFee, $fromDate, $toDate) {
        if ($orgStreamId == "no_one") {
            $iData = ["orgCourseId" => $orgCourseId, "streamId" => $streamId, "loginId" => $loginId, "description" => $description, "totalSheet" => $totalSheet, "seoKeywords" => $seoKeywords, "availableSheet" => $availableSheet
                , "courseFeeType" => $courseFeeType, "courseFee" => $courseFee, "registrationFee" => $registrationFee, "fromDate" => $fromDate, "toDate" => $toDate, "createdAt" => $timenow, "isactive" => 1];
            ...
            $uData = ["orgCourseId" => $orgCourseId, "streamId" => $streamId, "loginId" => $loginId, "description" => $description, "totalSheet" => $totalSheet, "seoKeywords" => $seoKeywords,
                "availableSheet" => $availableSheet, "courseFeeType" => $courseFeeType, "courseFee" => $courseFee, "registrationFee" => $registrationFee, "fromDate" => $fromDate, "toDate" => $toDate, "updatedAt" => $timenow];

            $response = $this->db->where("orgStreamId", $orgStreamId)->update("organization_streams", $uData);
            ($response ? $this->addActivityLog($_SESSION['loginId'], "Organization Stream Details Updated by " . $_SESSION['orgName'], "organization_streams", "0") : "");
            return ($response ? '{"status":"success", "msg":"Save Successful"}' : '{"status":"error", "msg":"Error in server, please contact admin!"}');
        }
    }

    public function mgetStreams() {
        $courseId = FILTER_VAR(trim($this->input->post('courseId')), FILTER_SANITIZE_STRING);
        if (!isset($_SESSION['loginId'])) {
            return '{"status":"error", "msg":"Unauthorised access!"}';
        }
        ...
        $sdata = $this->db->query("SELECT * FROM stream_details WHERE $condition");
        if ($sdata->num_rows() > 0) {
    ...
    public function mViewStreams() {
        ...
        $condition = ($orgStreamId != "" ? "and orgStreamId=$orgStreamId" : "");
        $response = $this->db->query("SELECT os.orgStreamId,os.orgCourseId,oc.courseId,os.description,os.seoKeywords,os.totalSheet,os.availableSheet,ct.courseType,cd.title as course
                                        ,td.title as timeduration,sd.title as stream,os.streamId,os.courseFeeType,os.courseFee,os.registrationFee,os.fromDate,os.toDate,
                                        CONCAT('Course Fee: ',os.courseFee,' Registartion Fee: ',os.registrationFee,'<br>Fee Submit From: ',DATE_FORMAT(os.fromDate,'%d-%b-%Y'),
                                        'Fee Submit To: ',DATE_FORMAT(os.toDate,'%d-%b-%Y')) feeDetails
                                        FROM organization_streams as os
        ...
        $result = ($response->num_rows() > 0 ? $response->result() : "");
        return json_encode($result);
    }

    public function mDelStreams() {
        $orgStreamId = FILTER_VAR(trim($this->input->post('del')), FILTER_SANITIZE_STRING);
        if (!isset($_SESSION['loginId']) || empty($orgStreamId)) {
            return '{"status":"error", "msg":"Empty Details!"}';
        } else {
            $data = ["orgStreamId" => $orgStreamId, "loginId" => $_SESSION['loginId'], "isactive" => 1];
            ...
            if ($resp === "notfound") {
                $this->addActivityLog($_SESSION['loginId'], "Organization Stream Details Deleted by " . $_SESSION['orgName'], "organization_streams", "0");
    ...
    public function mgetClassType() {
        ...
        $response = ($data->num_rows() > 0 ? $data->result() : "");
        return json_encode($response);
    }
    ...
    public function maddEligibility() {
        ...
        $loginId = $_SESSION['loginId'];
        if (empty($loginId) || empty($minQualificationId) || empty($courseTypeId) || empty($courseId) || empty($class) || empty($classTypeId) || empty($percentage)) {
            return '{"status":"error", "msg":"Empty Details!"}';
        } else {
            if ($minQualificationId === "no_one") {
                return $this->addEligibility($loginId, $courseTypeId, $courseId, $class, $classTypeId, $percentage);
            } else {
                return $this->upDateEligibility(base64_decode($minQualificationId), $loginId, $courseTypeId, $courseId, $class, $classTypeId, $percentage);
            }
        }
    }

    private function addEligibility($loginId, $courseTypeId, $courseId, $class, $classTypeId, $percentage) {
        $chkData = ["loginId" => $loginId, "courseTypeId" => $courseTypeId, "courseId" => $courseId, "class" => $class,
        ...
        if ($duplicate->num_rows() > 0) {
            ...
            if ($res) {
                $this->addActivityLog($_SESSION['loginId'], "Eligibility Inserted by " . $_SESSION['orgName'], "minimum_qualification", "0");
                return '{"status":"success", "msg":"Save Successful"}';
            } else {
                return '{"status":"error", "msg":"Error in server, please contact admin!"}';
            }
    ...
    private function upDateEligibility($minQualificationId, $loginId, $courseTypeId, $courseId, $class, $classTypeId, $percentage) {
        $chkData = ["minQualificationId!=" => $minQualificationId, "loginId" => $loginId, "courseTypeId" => $courseTypeId, "courseId" => $courseId, "class" => $class,
            "classTypeId" => $classTypeId, "percentage" => $percentage, "isactive" => 1];
        $this->db->where($chkData);
        $duplicate = $this->db->get("minimum_qualification");
        if ($duplicate->num_rows() > 0) {
    ...
    public function mShowEligibility() {
        ...
        $condition = ($Id != "" ? "AND mq.minQualificationId=" . base64_decode($Id) . "" : "");

        $result = $this->db->query("SELECT mq.minQualificationId,mq.loginId,mq.courseTypeId,mq.courseId,mq.courseTypeId,mq.percentage,ct.courseType,
                                cd.title,sct.class,sct.title as classtitle,mq.classTypeId FROM minimum_qualification mq
                                INNER JOIN course_type ct ON ct.ctId=mq.courseTypeId AND ct.isactive=1
                                INNER JOIN course_details cd ON cd.cId=mq.courseId AND cd.isactive=1
                                INNER JOIN school_class_type sct ON sct.classTypeId=mq.classTypeId
                                WHERE mq.loginId=" . $_SESSION['loginId'] . " AND mq.isactive=1 $condition");
        $response = ($result->num_rows() > 0 ? $result->result() : "");
        if ($response) {
    ...
    public function mgetNews() {
        ...
        $response = $this->db->query("SELECT newsId,loginId,title,description,newsImage,DATE_FORMAT(publishDate,'%d-%b-%y') AS publishDate
    ...
    public function mPlacedStudentRecords() {
        ...
        $qry = $this->db->query("SELECT tp.*,CONCAT('Placed On:',DATE_FORMAT(tp.placementDate,'%d-%b-%Y'),' Package: ',tp.package,' (',cr.code,'(',cr.name,'))') placementDetails
                                                        FROM tbl_placedstudents tp
                                                        LEFT JOIN currencies cr ON cr.code=tp.currency
    ...
    public function mRunningStatus() {
        ...
        $title = FILTER_VAR(trim($this->input->post('title')), FILTER_SANITIZE_STRING);
        if (empty($title) || empty($loginId) || empty($runningStatusIds)) {
            return '{"status":"error", "msg":"Empty Details!"}';
        } if ($runningStatusIds !== "no_one") {
            $condition = ["runningStatusId!=" => base64_decode($runningStatusIds), "loginId" => $loginId, "title" => $title, "isactive" => 1];
        } else {
            $condition = ["loginId" => $loginId, "title" => $title, "isactive" => 1];
        }
        $this->db->where($condition);
        $chkduplicate = $this->db->get("running_status");
        if ($chkduplicate->num_rows() > 0) {
            return '{"status":"error", "msg":"Duplicate entry!"}';
        }
        return $this->insertUpdateRunningStatus($runningStatusIds, $loginId, $title);
    }

    private function insertUpdateRunningStatus($runningStatusIds, $loginId, $title) {
        if ($runningStatusIds === "no_one") {
            ...
            $this->db->where(["runningStatusId" => base64_decode($runningStatusIds), "loginId" => $loginId]);
            $res = $this->db->update("running_status", ["title" => $title, "updatedAt" => $this->datetimenow()]);
            $this->addActivityLog($_SESSION['loginId'], "Running Status Updated by " . $_SESSION['orgName'], "running_status", "0");
        } if ($res) {
            return '{"status":"success", "msg":"Saved Successful"}';
        } else {
            return '{"status":"error", "msg":"Error in server, please contact admin!"}';
        }
    }
    ...
    public function mgetAllEnrollments($condition) {
        ...
        return $this->db->query("SELECT te.enrollmentId,te.status,oc.courseDurationType,DATE_FORMAT(oc.openingDate ,'%d-%b-%Y') openingDate,
                                        ...
                                        INNER JOIN organization_streams os ON os.orgStreamId=te.orgStreamId
                                        INNER JOIN stream_details sd ON sd.streamId=os.streamId INNER JOIN organization_courses oc ON oc.orgCourseId=os.orgCourseId
                                        LEFT JOIN department dep ON dep.departmentId=oc.departmentId AND dep.isactive=1 INNER JOIN course_type ct ON ct.ctId = oc.courseTypeId AND ct.isactive=1
                                        INNER JOIN course_details cd ON cd.cId=oc.courseId AND cd.isactive=1 INNER JOIN time_duration td ON td.tdId=oc.courseDurationId AND td.isactive=1
                                        INNER JOIN login_details ld ON ld.id=oc.loginId AND ld.isactive=1  INNER JOIN organization_details od ON ld.id=od.loginId AND od.isactive=1
                                        WHERE  te.isactive=1 AND oc.loginId=" . $_SESSION['loginId'] . " $condions");
    }
    ...
    public function mChangeStatus() {
        ...
        if ($chk->num_rows() > 0) {
            $resp = $this->db->where("enrollmentId", $enrollmentId)->update("tbl_enroll", ["updatedAt" => $this->datetimenow(), "status" => $status]);
            ...
            return '{"status":"error", "msg":"No details."}';
        }
    }
    ...
    public function mGetCourseNames() {
        $qry = $this->db->query("SELECT CONCAT(cd.title,'(',ct.courseType,' [',td.title,']',')') courseName,oc.orgCourseId FROM organization_courses oc
                                ...
                                INNER JOIN time_duration td ON td.tdId=oc.courseDurationId AND td.isactive=1
    ...
    private function addActivityLog($user_id, $activity, $act_table, $isadmin) {
        $idata = ["user_id" => $user_id, "activity" => $activity, "act_table" => $act_table, "date" => date('Y-m-d'), "isadmin" => 0,
            "role_name" => "College", "created_at" => $this->datetimenow(), "ip_address" => $this->getRealIpAddr(), "isactive" => 1];
